...
class ProjectileManager extends BaseEntityManager {
    ...
    generateProjectile = (position: Position, angle: number, colour: string, velocity: number): Projectile => {
        ...
        return new Projectile(5, 5, {x: position.x, y: position.y}, colour, velocityX, velocityY)
    }
    ...
    addProjectile = (location: Position, angle: number, colour: string, velocity: number): void => {
